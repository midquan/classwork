...
timer_sleep (int64_t ticks) 
{
  ...
  list_push_back(&sleep, &thread_current()->this_elem);
  
  // 1. Save wakeup time (start + ticks) in current thread
  // 1.1 Add a new field to the thread structure in thread.h
  // 1.2 Set that new field here 
  // 2. Call whatever function puts the current thread to sleep
  enum intr_level old_level = intr_disable();
  thread_block();
  //if(time_ticks () < start + ticks) intr_enable();
  intr_set_level(old_level);
}
...
void timer_wakeup(void){
	struct list_elem *my_elem = list_begin(&sleep);
	while(my_elem != list_end(&sleep)){
		struct thread *t = list_entry(my_elem, struct thread, this_elem);
		//wakeup
