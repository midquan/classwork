...
//Constructs a node based on given values
...
//Constructs a List based on given values
List newList(void){
	List L; 
...
int get(List L){
	if(L == NULL){
      fprintf(stderr, "Cannot call get() from NULL list reference. \n");
      exit(1);
   }
	if(L->numData < 1 || L->index < 0){
...
int equals(List A, List B){
	...
	if( A->numData != B->numData ) return eq;
	test1 = A->front;
	test2 = B->front;
	while(test1->next != NULL && test2->next != NULL){
      eq = (test1->data==test1->data);
      test1 = test1->next;
      test2 = test2->next;
   }
	return eq;
...
void moveNext(List L){
	...
	if(L->cursor != NULL && L->index != L->numData - 1) {
      L->cursor = L->cursor->next;
      ++L->index;
   }
   else if(L->cursor != NULL && L->index == L->numData - 1) {
      L->cursor = NULL;
      L->index = -1;
   }
}
